...
#define RELAY_NO_PIN 10                   //Normally Open(NO) pin in relay module
...
void setup() {
  ...
  digitalWrite(RELAY_NO_PIN, HIGH);          //initially turn relay off
...
void loop() {
  ...
  if ( dis <= THRESHOLD_DISTANCE) //if object is currently in range
  {
    ...
    if (!wasInRangePrevious)                //if object was not in range previously
    {
      ...
      if (getDistance() <= THRESHOLD_DISTANCE) //if object is still in range
      {
